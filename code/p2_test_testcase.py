...
def predict(args, model, data_loader, m_augment=100):
    ...
    with torch.no_grad():
        ...
        for i, (data, target) in enumerate(data_loader):
            ...
            #print(x.shape)
            x = x.reshape(args.N_shot+m_augment, args.N_way, -1).mean(dim=0)
            p = x
            x = model.encoder(query_input)
            x = x.view(x.size(0), -1)
...
def parse_args():
    ...
    parser.add_argument('--load', type=str, help="Model checkpoint path", default='./p2-max-acc.pth')
